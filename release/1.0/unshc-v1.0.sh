...
#  04/13/23 Matches version 7.5.0 of GCC
###################
# Tested on :
...
function clean(){
        $SHRED -zu -n 1 $DUMPFILE $CALLFILE $CALLFILE2 $CALLADDRFILE $CALLSIZEFILE $STRINGFILE $TMPBINARY ${TMPBINARY}.c >/dev/null 2>&1
}
...
# Update 04/13/2023 : Matches version 7.5.0 of GCC
function extract_arc4_call_addr(){
	TAILNUMBER=$1
	CALLADDRS=$($GREP -Eo "call.*[0-9a-f]{2,}" $DUMPFILE | $GREP -Eo "[0-9a-f]{2,}" | $SORT | $UNIQ -c | $SORT | $GREP -Eo "(15).*[0-9a-f]{2,}" | $GREP -Eo " [0-9a-f]{2,}" | $GREP -Eo "[0-9a-f]{2,}")
...
function extract_variables_from_binary(){
        ...
        while [[ $($WC -l < $CALLADDRFILE) -ne 14 ]]; do
                $GREP -B $i "call.*$CALLADDR" $DUMPFILE | $GREP -v "$CALLADDR" | $GREP "lea" | $GREP -Eo "(# [0-9a-f]{6,})" | $GREP -Eo "([0-9a-f]{6,})" > $CALLADDRFILE
                i=$(($i + 1))
                if [ $i -eq 10 ]; then
        ...
        while [[ $($WC -l < $CALLSIZEFILE) -ne 14 ]]; do
                $GREP -B $i "call.*$CALLADDR" $DUMPFILE | $GREP -v "$CALLADDR" | $GREP -Eo "(0x[0-9a-f]+,)" | $GREP -Eo "(0x[0-9a-f]+)" | $GREP -Ev "0x[0-9a-f]{6,}" | $TAIL -n +2 > $CALLSIZEFILE
                i=$(($i + 1))
                if [ $i -eq 10 ]; then
        ...
        do
                ...
                #  mov    $0x<hex>,%esi
                #  lea    $0x<adr>,%edi
                #  callq  $CALLADDR <__cxa_finalize@plt+0x17b>
                #
...
function extract_password_from_binary(){
        ...
        while [[ ( -z "$KEY_ADDR" ) || ( -z "$KEY_SIZE" ) ]]; do
                $GREP -B $i -m 2 "call.*$CALLADDR" $DUMPFILE | $GREP -v $CALLADDR > $CALLFILE
                FIRST_CALL_LEN=$($GREP -n "call" $CALLFILE | $HEAD -n 1 | $CUT -d ':' -f 1)
                FIRST_CALL_LEN=$(($FIRST_CALL_LEN + 1))
                echo $FIRST_CALL_LEN
                $TAIL -n +$FIRST_CALL_LEN $CALLFILE > $CALLFILE2
                # cat $CALLFILE
                # Adjust these two next line to grep right addr & size value (depending on your architecture)
                KEY_ADDR=$($GREP -B 3 -m 1 "call" $CALLFILE2 | $GREP -Eo "(# [0-9a-f]{6,})" | $GREP -Eo "([0-9a-f]{6,})" | $HEAD -n 1)
                KEY_SIZE=$($GREP -B 3 -m 1 "call" $CALLFILE2 | $GREP mov | $GREP -v $KEY_ADDR | $GREP -v movb | $GREP -oE "0x[0-9a-z]+" | $HEAD -n 1)
                i=$(($i + 1))
                if [ $i -eq 10 ]; then
